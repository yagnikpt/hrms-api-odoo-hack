...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.CheckConstraint("status IN ('PRESENT', 'ABSENT', 'HALF_DAY', 'LEAVE')", name='ck_attendance_records_status'),
    ...
    op.create_table(
        ...
        sa.Column('allowances', sa.Numeric(precision=12, scale=2), nullable=False, server_default=sa.text('0')),
        sa.Column('deductions', sa.Numeric(precision=12, scale=2), nullable=False, server_default=sa.text('0')),
        sa.Column('effective_from', sa.Date(), nullable=False),
        sa.Column('created_at', sa.TIMESTAMP(timezone=True), nullable=False, server_default=sa.text('now()')),
        sa.ForeignKeyConstraint(['employee_id'], ['employees.id'], ondelete='CASCADE'),
        sa.UniqueConstraint('employee_id', 'effective_from', name='uq_salary_structures_employee_effective_from'),
    )
    ...
    op.create_table(
        ...
        sa.UniqueConstraint('employee_id', 'month', name='uq_salary_slips_employee_month'),
    )

    op.create_index('idx_attendance_employee_date', 'attendance_records', ['employee_id', 'work_date'])
